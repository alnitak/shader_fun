/// Installs, updates, or checks the shader_fun agent skills.
///
/// Running `dart run shader_fun:skills` is itself the consent to write
...
// The bundled agent skills teaching correct shader_fun usage each live at
...
Future<void> main(List<String> args) async {
  if (args.contains('-h') || args.contains('--help')) {
    stdout.writeln(
      'Usage: dart run shader_fun:skills [--check]\n'
      '\n'
      'Installs or updates the shader_fun agent skills (correct-usage '
...
String describeSkillPlan(SkillInstallPlan plan) {
  switch (plan.action) {
    case SkillInstallAction.sourceMissing:
      return 'Could not locate the bundled shader_fun skills. This '
          'package version may predate them, or it is a git dependency '
          'published without them.';
    case SkillInstallAction.upToDate:
      return 'The shader_fun agent skills are up to date '
          '(${plan.skillNames.length} installed).';
    case SkillInstallAction.install:
      return '${plan.installCount} of the shader_fun agent skills '
          '(${plan.skillNames.length} total) are not installed. Install them '
          'with: dart run shader_fun:skills';
    case SkillInstallAction.update:
      return '${plan.updateCount} shader_fun agent skill(s) have a newer '
          'version available. Update with: dart run shader_fun:skills';
  }
}
...
Future<String> installSkills({Directory? projectRoot, Uri? skillsRoot}) async {
  ...
  final bundled = rootUri == null
      ...
      : _bundledSkills(rootUri);
  if (bundled.isEmpty) {
    return 'Could not locate the bundled shader_fun skills; skipped '
        'skill install.';
  }
  ...
  return 'Installed $count shader_fun agent skill${count == 1 ? '' : 's'} '
      'into ${homes.join(', ')}.';
}
...
Future<Uri?> _resolveBundledSkillsRoot() async {
  final lib = await Isolate.resolvePackageUri(
    Uri.parse('package:shader_fun/shader_fun.dart'),
  );
